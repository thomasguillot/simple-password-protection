...
**XML-RPC is switched off entirely while the gate is up**, not merely gated. The
...
This one genuinely is all-or-nothing: `xmlrpc.php` empties `$_COOKIE` before loading
...
`spp_client_ip` filters the IP address used for throttling. It receives a validated IP string, or an empty string when `REMOTE_ADDR` is missing or invalid — never the bucketed value that is actually counted (a prefix like `2001:db8:1:2::/64` for IPv6). Whatever it returns is passed through that same bucketing afterwards, so the reverse-proxy example below can safely return a plain validated address without masking it itself.
...
The exemption is based on the `edit_others_posts` capability, not merely on being logged in, and not on `edit_posts`. That distinction matters more than it looks. `edit_posts` is WordPress's "can write" capability, not "can read everything": a Contributor or an Author holds it *without* `edit_others_posts`, so wp-admin shows them nothing of anyone else's posts. Exempting them would have let `/wp-json/wp/v2/posts` return every published post in full to an account that cannot even load your home page without the password — REST handing out what the front door refuses.
...
That is a real trade, not a formality: it gives every Contributor read access to all published content on the site.
...
add_filter(
	'spp_client_ip',
	function ( $ip ) {
		// Cloudflare. Use the header your own proxy actually sets.
...
IPv6 addresses are counted per `/64`, not per address. A single IPv6 address is a misleading thing to rate-limit: the smallest block anyone is actually given is a `/64`, so a `$5` virtual server comes with 18 quintillion addresses and five guesses on each of them would not be a limit at all. Counting the `/64` means one allocation gets one budget. IPv4 is counted whole.

The flip side is the honest weakness of any IP-based throttle, and it is worth knowing before you rely on it: everyone sharing one IPv4 address shares one budget. Mobile networks, office and school NATs and VPN exits routinely put thousands of people behind a single address, and somebody on that address making five wrong guesses every fifteen minutes can keep everyone else on it locked out for as long as they care to. The gate is not a login system and the throttle is not an intrusion-prevention system; the thing that actually protects the site is the password being strong enough that guessing it is hopeless.

Windows are fixed rather than rolling, and the lockout lasts until the end of the current 15-minute window — so the wait is somewhere between a few seconds and 15 minutes, not always 15. The upside is that rolling over to a new window involves no cleanup step for concurrent requests to race on. The trade is that an attacker timing a burst across a boundary can get up to ten guesses in quick succession rather than five. That still makes online guessing impractical, but treat the throttle as what it is — a speed bump — and choose a password strong enough that it does not matter.
